...
class MOE(nn.Module):
    ...
    """
    # def __init__(self, input_size, output_size, num_experts, hidden_size, noisy_gating=True, k=10, device='cpu'):
    def __init__(self, input_size, output_size, num_experts, hidden_size, noisy_gating=True, k=3, device='cpu'):
    ...
    def forward(self, x, loss_coef=1e-2):
        ...
        i = 0
        expert_out2 = []
        expert_outputs2 = torch.stack(expert_outputs,dim=1)
        
        gates = gates.unsqueeze(-1)
        expert_outputs2 = expert_outputs2.mul(gates)
        y = expert_outputs2.sum(axis=1, keepdim=False)
        
        # expert_out2 = [e.mul(gates) for e in expert_outputs]

       
        # y = dispatcher.combine(expert_outputs)
        return y, loss, expert_outputs
